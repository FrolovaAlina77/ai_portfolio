  ...
  return {
    ...
    configureServer(server: import('vite').ViteDevServer) {
      server.middlewares.use((req, res, next) => {
        ...
        if (!file.startsWith(resolve(root, folder)) || !existsSync(file) || !statSync(file).isFile()) return next()
    ...
    closeBundle() {
      for (const folder of mediaFolders) {
        ...
        if (existsSync(source)) cpSync(source, resolve(root, 'dist', folder), { recursive: true })
      }
    },
  }
}
